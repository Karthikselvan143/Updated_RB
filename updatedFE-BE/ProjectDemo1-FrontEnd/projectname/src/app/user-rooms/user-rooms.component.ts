...
export class UserRoomsComponent implements OnInit {
  ...
  constructor(private http: HttpClient, private modalService: BsModalService, private fb: FormBuilder, private service:ServiceService) {
  ...
  ngOnInit(): void {
    this.service.get('Room/GetRooms').subscribe({
      next: (data) => {
        this.rooms = data;
  ...
  processPayment(): void {
    ...
    this.service.post('Payment/ProcessPayment', payload)
      .subscribe({
        next: (response: any) => {
